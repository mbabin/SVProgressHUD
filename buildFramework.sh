...
PROJECT=SVProgressHUD.xcodeproj
SCHEME=SVProgressHUD-Framework
FRAMEWORK_NAME=SVProgressHUD
...
xcodebuild archive -project "${PROJECT}" -scheme "${SCHEME}" -destination 'generic/platform=iOS' -archivePath "${PRODUCTS_IOS_PATH}/${DEVICE_ARCHIVE}" SKIP_INSTALL=NO | xcpretty

echo "XCFramework: Archiving SIMULATOR type..."
xcodebuild archive -project "${PROJECT}" -scheme "${SCHEME}" -destination 'generic/platform=iOS Simulator' -archivePath "${PRODUCTS_IOS_PATH}/${SIM_ARCHIVE}" SKIP_INSTALL=NO | xcpretty
